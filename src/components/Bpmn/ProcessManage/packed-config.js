...
var endEventRequired = function () {
  function hasEndEvent(node) {
    ...
    return flowElements.some((node) => is$1(node, 'bpmn:EndEvent'))
  }
  ...
  function check(node, reporter) {
    ...
    if (
      ...
      if (flagLeft) {
        reporter.report(
          node.id,
          translate(type) + newname + translate('Element is not left connected'),
        )
      }
      if (flagRight) {
        reporter.report(
          node.id,
          translate(type) + newname + translate('Element is not right connected'),
        )
      }
      // if (!incoming.length) {
...
var startEventRequired = function () {
  function hasStartEvent(node) {
    ...
    return flowElements.some((node) => is$9(node, 'bpmn:StartEvent'))
  }
